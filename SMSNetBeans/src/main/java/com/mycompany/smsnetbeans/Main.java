    ...
    private static void addStudent() {
        try {
            if (studentCount < students.length) {
                ...
                if (age <= 0 || age > 120) throw new IllegalArgumentException("Invalid age. Age must be between 1 and 120");
    ...
    private static void addTeacher() {
        try {
            if (teacherCount < teachers.length) {
                ...
                String name = scanner.nextLine().trim();
                if (name.isEmpty()) throw new IllegalArgumentException("Teacher name cannot be empty");

                int age = getIntInput("Enter teacher age: ");
                if (age < 21 || age > 70) throw new IllegalArgumentException("Invalid age. Age must be between 21 and 70");
